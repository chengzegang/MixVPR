...
def train(
    ...
    hostname = os.uname()[1]
    wandb.init(project="VPR", name=f"MixVPR-{dataset_name}-{hostname}", id=f"MixVPR-{dataset_name}-{hostname}")
    image_size = 320
    ...
    for epoch in range(epochs):
        dl = DataLoader(
            MixVPRVisualPlace.from_visual_place(dataset.sample(10000), transform=t), batch_size=32, shuffle=True, num_workers=1, collate_fn=collate_fn
        )
        result = eval_fn(
            model,
